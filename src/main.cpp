...
vector<vector<double>> warp_process_image(Mat& img)
{
	...
	threshold(planes[1],lane, 30, 255, THRESH_BINARY);
	imshow("lane", lane);
...
int main()
{
	...
	// row = 480, col = 640
	// warx_margin = 20, warpy_margin = 3
	/*
	warp_src[0] = cv::Point2f(200 - warpx_margin, 300 - warpy_margin);
	warp_src[1] = cv::Point2f(30  - warpx_margin, 400 + warpy_margin);
	warp_src[2] = cv::Point2f(440 + warpx_margin, 300 - warpy_margin);
	warp_src[3] = cv::Point2f(610 + warpx_margin, 400 + warpy_margin);
	*/

	// 설정 중요!!!
	// 캘리브레이션 필!!!
	warp_src[0] = cv::Point2f(200 - warpx_margin, 300 - warpy_margin);
	warp_src[1] = cv::Point2f(100 - warpx_margin, 390 + warpy_margin);
	warp_src[2] = cv::Point2f(640-180 + warpx_margin, 300 - warpy_margin);
	warp_src[3] = cv::Point2f(540 + warpx_margin, 390 + warpy_margin);
